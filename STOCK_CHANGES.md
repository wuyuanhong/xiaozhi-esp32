...
> 每次修改代码都追加更新此文档。最后更新：2026-07-21
...
**个股报警：**
- 条件：当前价格 vs 上次报警时价格变化 ≥ ±3%
- 冷却：10分钟/只
- 效果：喇叭提示音（OGG_POPUP）+ 该股票标签反色闪烁（黑底白字）
- 只在空闲状态报警
...
- `CONFIG_ESP_TIMER_TASK_STACK_SIZE` 从 3584 → 8192
- 解决股票页 LVGL 对象过多导致的栈溢出崩溃

#### 3. 刷新提示修复

- 新增 `stock_refresh_label_`（股票页自己的 label，不依赖天气页的 chat_status_label_）
- 长按 BOOT 时显示"刷新中..."在指数栏右侧
- 数据获取完成后自动隐藏

#### 4. 移除涨跌幅排序

- 移除 `SortAndRefreshList` 功能
- 股票列表保持固定顺序（添加时的顺序）
